...
- **Accuracy evaluation harness (M6) — built.** `eval/accuracy_benchmark.py` runs a labeled test set (`eval/test_set/`) through the real production pipeline (`recognize_page`, matching what live users get, including the PaddleOCR/Groq fallback) and computes character-level accuracy via Levenshtein edit distance against the SRS's NFR-010 target (≥80%), with per-image and aggregate results saved to `eval/test_set/results.json`. Seeded with 3 real pages (one each: poli-sci prose, calculus, econ) pulled from the actual course PDFs uploaded earlier this project, transcribed by hand against the real images. **Still needs to grow to the full 30-50 image target** (see `eval/test_set/README.md` for how to add more) before the resulting percentage is a fully defensible sample size for a resume line — but the harness itself, and a real first number, now exist where neither did before.
